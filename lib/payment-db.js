...
export async function excluirEventoPagamento(id) {
  await withSchema()

  const activePool = ensurePool()
  const result = await activePool.query('DELETE FROM payment_eventos WHERE id = $1', [String(id)])

  return result.rowCount > 0
}

export async function atualizarEventoPagamentoPorPaymentId(paymentId, atualizacoes) {
  await withSchema()

  const activePool = ensurePool()
  const result = await activePool.query(
    `SELECT id, data_evento, payload
     FROM payment_eventos
  ...
  const payload = row.payload && typeof row.payload === 'object' ? row.payload : {}
  const nextPayload = { ...payload, ...atualizacoes }
  const dataEvento = atualizacoes?.dataEvento ?? new Date().toISOString()

  delete nextPayload.id
  delete nextPayload.dataEvento

  await activePool.query(
    'UPDATE payment_eventos SET data_evento = $2, payload = $3::jsonb WHERE id = $1',
    [row.id, dataEvento, JSON.stringify(nextPayload)]
  )
